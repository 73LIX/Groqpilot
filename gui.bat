...
IF %ERRORLEVEL% NEQ 0 (
    echo Python is not installed. Downloading and installing Python 3.10.12...

    :: Download Python installer
    powershell -Command "Invoke-WebRequest -Uri https://www.python.org/ftp/python/3.10.12/python-3.10.12-amd64.exe -OutFile python-3.10.12-amd64.exe"

    :: Run the installer
    start /wait python-3.10.12-amd64.exe /quiet InstallAllUsers=1 PrependPath=1

    :: Deleting installer
    del python-3.10.12-amd64.exe >nul 2>&1
...
:wait_for_exit
echo Type "exit" and press Enter to stop the Gradio app.
